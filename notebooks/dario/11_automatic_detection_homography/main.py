...
def detect_robots_hsv(frame_bgr: np.ndarray,
    ...
    Devuelve sv.Detections con class_id: 0=azul, 1=rojo, 2=balon.
    """
...
OUTPUT_VIDEO = "assets/futbot_nb11.avi"
...
frame0 = next(sv.get_video_frames_generator("assets/futbot-2s.mp4"))
out0 = callback_nb11(frame0, 0)
...
while True:
    ...
    if not ret:
        break
    writer.write(callback_nb11(frame, idx))
    idx += 1
...
def detect_with_yolo(frame_bgr: np.ndarray,
    ...
    """
    Detector genérico con YOLO.
    Devuelve sv.Detections — misma interfaz que detect_robots_hsv().
    """
    model = YOLO(model_path)
    results = model(frame_bgr, conf=conf, verbose=False)[0]
    return sv.Detections.from_ultralytics(results)

# Para usar en el pipeline de video, solo cambia UNA línea en callback_nb11:
#   dets = detect_robots_hsv(frame)   ←  detector de color (este notebook)
dets = detect_with_yolo(frame)    #←  YOLO genérico
