...
# results = model(cap, half=True, device='cuda')[0]
...
def mi_callback(frame: np.ndarray, _: int) -> np.ndarray:
    ...
    labels = [f"ID:{tid} | Frame: {frame_count[tid]}" for tid in det.tracker_id]
...
sv.process_video(
    source_path="assets/vehicles.mp4",
    target_path="assets/vehicles_extension.mp4",
    callback=mi_callback
)
print("Guardado: assets/vehicles_extension.mp4")
