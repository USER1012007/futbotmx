...
from domain.events import (
    BallRepositionEvent, CollisionEvent, FrameEvents, GoalEvent,
    InvalidGoalEvent, OffCourtEvent, PanicEvent, PassEvent,
    PossessionEvent, RemoveRobotEvent, RobotRepositionEvent, RobotStoppedEvent,
)
from domain.field import (
    crossed_left_goal_entry, crossed_right_goal_entry, inside_court,
    inside_goal_mouth_y, robot_inside_penalty_area,
)
from infra.event_bus import EventBus
from analysis.event_constants import *
from analysis.event_deduper import EventDeduper
...
class EventDetector:
    def __init__(self, event_bus: Optional[EventBus] = None, *, default_fps: float = 30.0) -> None:
        ...
        self._deduper = EventDeduper(EVENT_COOLDOWN_FRAMES)
    ...
    def detect(self, frame_result: FrameResult) -> FrameEvents:
        ...
        if possession_event is not None: events.eventos.append(possession_event)
        pass_event = self._detect_pass(possession_event, frame, timestamp_s)
        if pass_event is not None: events.eventos.append(pass_event)
        ...
        events.eventos = self._deduper.dedupe(events.eventos, frame)
        self._update_last_valid_positions(frame_result.robots, frame_result.ball)
        self._prev = frame_result
        if self._event_bus is not None: self._event_bus.publish("frame_events", events)
        return events
    ...
    def _detect_panic(self, frame_result: FrameResult, frame: int, timestamp_s: float) -> List[PanicEvent]:
        ...
        if changes > 2:
            return [PanicEvent(frame=frame, timestamp_s=timestamp_s, severity="system", reason="ruido_visual", frames_duration=1)]
        return []
        ...
        for index, robot_a in enumerate(robots):
            ...
            for robot_b in robots[index + 1 :]:
                ...
                if self._distance(pos_a, pos_b) < 20.0:
                    ...
                    events.append(CollisionEvent(frame=frame, timestamp_s=timestamp_s, position_cm=midpoint, severity="warning", robots=[robot_a, robot_b]))
        return events

    def _detect_possession(self, robots: List[Robot], ball: Optional[Ball], ball_position: Optional[Tuple[float, float]], frame: int, timestamp_s: float) -> Optional[PossessionEvent]:
        ...
        for robot in robots:
            if not self._is_ally(robot): continue
            robot_position = self._position(robot)
            if robot_position is None: continue
            distance = self._distance(robot_position, ball_position)
            if distance < closest_distance:
                closest_robot = robot
                closest_distance = distance
        previous_possession = self._last_possession_robot
        if previous_possession is not None and (closest_robot is None or previous_possession.id != closest_robot.id):
            self._pending_pass_from = previous_possession
        self._last_possession_robot = closest_robot
        if closest_robot is None: return None
        return PossessionEvent(frame=frame, timestamp_s=timestamp_s, robot=closest_robot, team=closest_robot.team_id, distance_cm=closest_distance)

    def _detect_pass(self, possession_event: Optional[PossessionEvent], frame: int, timestamp_s: float) -> Optional[PassEvent]:
        if possession_event is None or self._pending_pass_from is None: return None
        ...
        self._pending_pass_from = None
        if from_position is None or to_position is None: return None
        return PassEvent(frame=frame, timestamp_s=timestamp_s, from_robot_id=from_robot.id, to_robot_id=to_robot.id, from_team=from_robot.team_id, to_team=to_robot.team_id, distance_cm=self._distance(from_position, to_position))

    def _detect_off_court(self, frame_result: FrameResult, robots: List[Robot], ball_position: Optional[Tuple[float, float]], frame: int, timestamp_s: float) -> List[OffCourtEvent]:
        ...
        if frame_result.ball is not None and ball_position is not None and not self._inside_court(ball_position):
            ...
            if last_position is not None:
                events.append(OffCourtEvent(frame=frame, timestamp_s=timestamp_s, severity="warning", object_type="balon", object_id=frame_result.ball.id, last_position_cm=last_position))
        for robot in robots:
            position = self._position(robot)
            if position is None or self._inside_court(position): continue
            last_position = self._last_valid_positions.get(robot.id)
            if last_position is not None:
                events.append(OffCourtEvent(frame=frame, timestamp_s=timestamp_s, severity="warning", object_type="robot", object_id=robot.id, last_position_cm=last_position))
        return events
        ...
        for robot in robots:
            if robot.speed < 2.0:
                ...
                if frames > 30 and robot.id not in self._stopped_emitted:
                    ...
                    events.append(RobotStoppedEvent(frame=frame, timestamp_s=timestamp_s, robot=robot, frames_duration=frames, duration_s=frames / self._default_fps if self._default_fps else None))
            else:
                self._stopped_frames[robot.id] = 0
                self._stopped_emitted.discard(robot.id)
        return events

    def _detect_goal(self, ball: Optional[Ball], ball_position: Optional[Tuple[float, float]], frame: int, timestamp_s: float) -> Optional[GoalEvent]:
    ...
    def _detect_invalid_goal(self, goal_event: GoalEvent, robots: List[Robot], frame: int, timestamp_s: float) -> Optional[InvalidGoalEvent]:
        ...
        for robot in robots:
            ...
            if position is not None and self._robot_inside_penalty_area(position, area):
                return InvalidGoalEvent(frame=frame, timestamp_s=timestamp_s, position_cm=goal_event.position_cm, severity="danger", team=goal_event.team, reason="robot_en_area_penalti", infracting_robot=robot)
        return None
    ...
    def _detect_remove_robot(self, robots: List[Robot], frame: int, timestamp_s: float) -> List[RemoveRobotEvent]:
        ...
        for robot in robots:
            ...
            if robot.is_penalized and not was_penalized:
                ...
                events.append(RemoveRobotEvent(frame=frame, timestamp_s=timestamp_s, severity="danger", robot=robot, reason=reason, frames_penalization=robot.penalization_frames_left, penalization_s=robot.penalization_frames_left / self._default_fps if self._default_fps else None))
            self._prev_penalized[robot.id] = robot.is_penalized
        return events

    def _detect_repositions(self, frame_result: FrameResult, frame: int, timestamp_s: float) -> List[BallRepositionEvent | RobotRepositionEvent]:
        ...
        for reposition in getattr(frame_result, "repositions", []) or []:
            ...
            if event_type == "reposicion_balon" or object_type in {"ball", "balon"}:
                to_position = self._tuple_or_none(reposition.get("to_position_cm", reposition.get("target_position_cm", reposition.get("position_cm"))))
                if to_position is None: continue
                from_position = self._tuple_or_none(reposition.get("from_position_cm"))
                if from_position is None and frame_result.ball is not None: from_position = self._last_valid_positions.get(frame_result.ball.id)
                events.append(BallRepositionEvent(frame=frame, timestamp_s=timestamp_s, position_cm=to_position, severity="warning", reason=reposition.get("reason", "falta_de_progreso"), from_position_cm=from_position, to_position_cm=to_position))
                continue
            if event_type == "reposicion_robot" or object_type == "robot":
                ...
                target_position = self._tuple_or_none(reposition.get("target_position_cm", reposition.get("to_position_cm", reposition.get("position_cm"))))
                if robot is None or target_position is None: continue
                from_position = self._tuple_or_none(reposition.get("from_position_cm"))
                if from_position is None: from_position = self._last_valid_positions.get(robot.id)
                events.append(RobotRepositionEvent(frame=frame, timestamp_s=timestamp_s, position_cm=target_position, severity="warning", robot=robot, from_position_cm=from_position, target_position_cm=target_position))
        return events

    def _update_ball_contact(self, robots: Iterable[Robot], ball_position: Optional[Tuple[float, float]], frame: int) -> None:
        if ball_position is None: return
        closest_robot: Optional[Robot] = None
    ...
    def _goal_team_and_scorer(self, fallback_team: str, frame: int) -> Tuple[str, Optional[Robot]]:
        if self._last_ball_contact_robot is not None and self._last_ball_contact_frame is not None and frame - self._last_ball_contact_frame <= GOAL_CONTACT_MEMORY_FRAMES:
    ...
    def _update_last_valid_positions(self, robots: Iterable[Robot], ball: Optional[Ball]) -> None:
        for robot in robots:
            position = self._position(robot)
            if position is not None and self._inside_court(position): self._last_valid_positions[robot.id] = position
        if ball is not None:
            position = self._position(ball)
            if position is not None and self._inside_court(position): self._last_valid_positions[ball.id] = position

    def _is_robot_in_any_penalty_area(self, robot: Robot) -> bool:
        position = self._position(robot)
        return position is not None and (self._robot_inside_penalty_area(position, "ally") or self._robot_inside_penalty_area(position, "rival"))

    @staticmethod
    def _position(obj: object) -> Optional[Tuple[float, float]]:
        if obj is None: return None
        metric = getattr(obj, "position_metric", None)
        return (float(metric.x), float(metric.y)) if metric is not None else None

    @staticmethod
    def _distance(a: Tuple[float, float], b: Tuple[float, float]) -> float: return hypot(a[0] - b[0], a[1] - b[1])

    @staticmethod
    def _inside_court(position: Tuple[float, float]) -> bool: return inside_court(position)

    @staticmethod
    def _robot_inside_penalty_area(position: Tuple[float, float], area: str) -> bool: return robot_inside_penalty_area(position, area)
    ...
    def _tracker_ids(frame_result: FrameResult) -> Set[int]:
        ...
        if frame_result.ball is not None and frame_result.ball.tracker_id is not None: ids.add(frame_result.ball.tracker_id)
        return ids

    @staticmethod
    def _is_ally(robot: Robot) -> bool: return robot.team_id.lower() in {"allies", "ally", "azul", "blue"}

    @staticmethod
    def _team_name(team: object) -> str:
        if isinstance(team, str): return team.lower()
        return getattr(team, "name", str(team)).lower()

    @staticmethod
    def _tuple_or_none(value: object) -> Optional[Tuple[float, float]]:
        if value is None: return None
        if isinstance(value, dict): return (float(value["x"]), float(value["y"]))
        if isinstance(value, (list, tuple)) and len(value) >= 2: return (float(value[0]), float(value[1]))
        return None

    def _timestamp_s(self, frame_result: FrameResult) -> float:
        timestamp_s = getattr(frame_result, "timestamp_s", None)
        if timestamp_s is not None: return float(timestamp_s)
        return float(frame_result.frame_id) / self._default_fps if self._default_fps else 0.0
